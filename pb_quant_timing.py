...
def get_stock_pb(sec, sday, eday):
    temp = jzmongo['stock_raw.stock_valuation'].read(sec)\
...
if __name__ == '__main__':
    ...
    for stock_code in stock_code_lst:
        try:
            ...
            stock_pb = get_stock_pb(stock_code, s_day, e_day).reset_index(drop=True)
            lst_mid = []
            lst_up = []
            lst_down = []
            pe_ratio_lst = stock_pb.pb_ratio.tolist()
            for i in range(len(stock_pb)):
                if i < period:
                    lst_mid.append(None)
                    lst_up.append(None)
            ...
            stock_pb['pb_mid'] = lst_mid
            stock_pb['pb_up'] = lst_up
            stock_pb['pb_down'] = lst_down
            stock_hq = stock_hq.merge(stock_pb, on=['trade_date']).sort_values(['trade_date'])\
                .assign(close_1=lambda df: df.close.shift(1))\
                .assign(pb_ratio=lambda df: df.pb_ratio.shift(1)).dropna()
            net = 1
            net_lst = []
            pos_lst = []
            pos = 0
            ...
            for idx, _row in stock_hq.iterrows():
                if pos == 0:
                    if _row.pb_ratio < _row.pb_down:
                        cost = _row.open
                        pos = weight_long
                        net = (pos * _row.close / cost + (1 - pos)) * net
                        sell_method = 0
                elif (pos > 0) & (pos <= 1 - weight_long):
                    if (_row.low < cost * (1 - width_long)) & (_row.pb_ratio < _row.pb_mid):
                        ...
                        sell_method = 0
                    elif (sell_method == 0) & (_row.pb_ratio > _row.pb_up):
                        s_price = _row.open
                        ...
                        sell_method = 1
                        sell_price = s_price
                    elif (sell_method == 1) & (_row.pb_ratio > _row.pb_mid) & (_row.high > sell_price * (1 + width_short)):
                    ...
                    if (sell_method == 0) & (_row.pb_ratio > _row.pb_up):
                        s_price = _row.open
                        chg_pos = weight_short
                        net = net * (chg_pos * s_price / _row.close_1 + (pos - chg_pos) * _row.close / _row.close_1 + (
                                1 - pos))
            ...
            stock_hq.ix[:, ['pb_ratio', 'pb_mid', 'pb_up', 'pb_down']].plot()
            plt.rcParams['font.sans-serif'] = ['SimHei']
            plt.title('PB_BAND:' + stock_code[:-5])
            plt.savefig(fold + 'fig/' + stock_code[:-5] + 'PB_BAND' + '.png')
            plt.show()

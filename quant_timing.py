...
if __name__ == '__main__':
    ...
    pecent_lst = [i/100 for i in range(20, 96, 5)]
        ...
        for s_period in s_period_lst:
            for l_period in l_period_lst:
                ...
                for i in range(len(index_code_lst)):
                    ...
                    chg_df_['excess_chg' + str(i)] = chg_df_['chg' + str(i)] - chg_df_['close_chg' + str(i)]
                    chg_df_ = chg_df_.fillna(value=0).reset_index(drop=True)[
                        ['trade_date', 'chg' + str(i), 'close_chg' + str(i), 'excess_chg' + str(i)]]
    ...
    trd_state_df = pd.DataFrame(lst, columns=['indus_name', 'code', 'trd_time', 'hold_day', 'short_position', 'annR',
                                              'maxRetrace', 'sharp', 's_period', 'l_period', 'pecent', 's_date', 'e_date'])
    trd_state_df['ave_hold_day'] = trd_state_df['hold_day'] / trd_state_df['trd_time']
    trd_state_df.to_csv(fold + 'trd_state_df2.csv', encoding='gbk')

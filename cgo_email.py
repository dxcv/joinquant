...
if __name__ == '__main__':
    ...
    if today in calen:
        ...
        StartDate = calen[125]
        listDATE = calen[-125]
        listday_info = get_all_securities(types=['stock'], date=today).query("start_date<'{date}'".format(date=listDATE))
        ...
        HS300_stocks_list = [i for i in HS300_stocks_list if i in all_stocks_allowed]
        zz500_stocks_list = [i for i in zz500_stocks_list if i in all_stocks_allowed]
        ratio_list = list()
        values = {'cumprod1': 1}
        ...
        for i in SZ50_stocks_list:
            temp = values_data_cgo(i, 125, EndDate)
        ...
        for i in HS300_stocks_list:
            temp = values_data_cgo(i, 125, EndDate)
            df_300_turnover.append(temp)
        df_300_turnover = pd.concat(df_300_turnover)
        df_300_turnover.columns = ['tradedate', 'stockcode', 'turn_radio']
        df_300_turnover = df_300_turnover.assign(tradedate=lambda df: df.tradedate.apply(lambda x: str(x)[:10]))
        print(df_300_turnover)
        # 行情数据
        df_300_price = []
        for i in HS300_stocks_list:
            temp = stock_price_cgo(i, StartDate, EndDate)
            df_300_price.append(temp)
        df_300_price = pd.concat(df_300_price).assign(tradedate=lambda df: df.tradedate.apply(lambda x: str(x)[:10]))
        df_300_1 = df_300_turnover.merge(df_300_price, on=['tradedate', 'stockcode'])

        # 合成中证500成分股数据
        df_500_turnover = []

        for i in zz500_stocks_list:
            temp = values_data_cgo(i, 125, EndDate)
            df_500_turnover.append(temp)
        df_500_turnover = pd.concat(df_500_turnover)
        df_500_turnover.columns = ['tradedate', 'stockcode', 'turn_radio']
        df_500_turnover = df_500_turnover.assign(tradedate=lambda df: df.tradedate.apply(lambda x: str(x)[:10]))
        print(df_500_turnover)
        # 行情数据
        df_500_price = []
        for i in zz500_stocks_list:
            temp = stock_price_cgo(i, StartDate, EndDate)
            df_500_price.append(temp)
        df_500_price = pd.concat(df_500_price).assign(tradedate=lambda df: df.tradedate.apply(lambda x: str(x)[:10]))
        df_500_1 = df_500_turnover.merge(df_500_price, on=['tradedate', 'stockcode'])
        ...
        df_300_1 = df_300_1[['stockcode', 'tradedate', 'turn_radio', 'close', 'volume', 'money']]
        df_300_1.columns = ['STOCKCODE', 'ENDDATE', 'TURNOVER_DAY', 'TCLOSE', 'TVOLUME', 'TVALUE']
        df_300_1 = df_300_1.reset_index(drop=True)\
            .assign(avgprice=lambda df: df.TVALUE/df.TVOLUME)\
            .assign(TURNOVER_DAY=lambda df: df.TURNOVER_DAY/100)\
            .assign(comturnover=lambda df: 1-df.TURNOVER_DAY.shift(-1))

        df_500_1 = df_500_1[['stockcode', 'tradedate', 'turn_radio', 'close', 'volume', 'money']]
        df_500_1.columns = ['STOCKCODE', 'ENDDATE', 'TURNOVER_DAY', 'TCLOSE', 'TVOLUME', 'TVALUE']
        df_500_1 = df_500_1.reset_index(drop=True)\
            .assign(avgprice=lambda df: df.TVALUE/df.TVOLUME)\
            .assign(TURNOVER_DAY=lambda df: df.TURNOVER_DAY/100)\
            .assign(comturnover=lambda df: 1-df.TURNOVER_DAY.shift(-1))
        ...
        for name, group in df_300_1.groupby('STOCKCODE'):
            # temp = group.reset_index(drop=True).assign(rpt=lambda df: [
            #     rpt(df.ix[get_index(i):i, ['ENDDATE', 'TCLOSE', 'comturnover', 'TURNOVER_DAY', 'avgprice']], N) for i in
            #     range(1, len(group) + 1)]).assign(STOCKCODE=name)
            temp = group.reset_index(drop=True).assign(rpt=lambda df: [
                rpt(df.iloc[get_index(i):i, :], N) for i in
                range(1, len(group) + 1)]).assign(STOCKCODE=name)

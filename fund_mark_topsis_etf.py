...
def stock_price(sec, period, sday, eday):
    ...
    输出 个股的后复权的开高低收价格
    """
    temp = get_price(sec, start_date=sday, end_date=eday, frequency=period, fields=['open', 'close', 'high', 'low', 'money'],
...
if __name__ == '__main__':
    ...
    # fund_id 为符合条件的基金名单
    num = 0
    fund_id = pd.read_csv(fold + 'etf.csv', encoding='gbk', index_col=0)
    fund_id = fund_id.dropna()
    value_start = '2015-01-19'
    eday = '2020-02-20'
    fund_id_lst = fund_id.index.tolist()
    # df = []
    # for i in range(len(fund_id_lst)):
    #     print(fund_id_lst[i])
    #     temp = stock_price(fund_id_lst[i], 'daily', value_start, eday)
    #     temp = temp.dropna()
    #     print(temp)
    #     df.append(temp)
    # df = pd.concat(df)
    # df.to_csv(fold + 'etf_value.csv')
    df = pd.read_csv(fold + 'etf_value.csv', index_col=0, encoding='gbk')
    df = df.rename(columns={'stockcode': 'code'})
    # 计算每只基金评价指标
    #      fund_lst=fund_id.main_code.drop_duplicates().tolist()
    # 业绩基准序列
    d = df.tradedate.drop_duplicates().tolist()
    d = [str(i) for i in d]
    d.sort(reverse=True)
    s = d[-1]
    e = d[0]
    ...
    fund_id['code'] = fund_id.index
    id_name = fund_id[['code', 'display_name', 'name', 'start_date', 'class', 'type']]
    ...
    # 基金筛选
    # fund_ret_get = fund_ret.query("annROR < 10000000")
    fund_ret_get = fund_ret
    fund_ret_get = fund_ret_get[['code', 'display_name', 'name', 'start_date', 'class', 'type', 'annROR', 'avgRetrace', 'Var', 'IR']]
    zb_list = ['annROR', 'avgRetrace', 'Var', 'IR']
    direc = [1, -1, 1, 1]
    fund_ret_get = topsis(fund_ret_get, zb_list, direc, w=[2, 1, 1, 1])
    ...
    fund_ret_get.to_excel(fold + 'fund_mark_etf_topsis_zf' + '.xls', encoding='gbk')

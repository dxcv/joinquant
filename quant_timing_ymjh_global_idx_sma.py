...
if __name__ == '__main__':
    ...
    for j in range(len(index_code_lst)):
        ...
        for i in range(len(para_lst)):
            ...
            N2 = para[1]
            position = index_hq \
                         .assign(ma_s=lambda df: tb.SMA(df.close.shift(1).values, N1))\
                         .assign(ma_l=lambda df: tb.SMA(df.close.shift(1).values, N2))\
            ...
            for idx, _row in position.iterrows():
                condition_l = (_row.ma_s > _row.ma_l) and (_row.ma_s1 < _row.ma_l1)
                condition_s = (_row.ma_s1 > _row.ma_l1) and (_row.ma_s < _row.ma_l)
                if pos == 0:
                    if condition_l:
                        pos = 1
                elif pos == 1:

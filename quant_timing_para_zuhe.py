...
if __name__ == '__main__':
    ...
    for j in range(len(index_code_lst)):
        ...
        chg_df = chg_df.sort_values(['trade_date']).set_index(['trade_date'])
        chg_df['chg' + index_name] = chg_df.sum(axis=1) / len(para_lst) - chg_df['close_chg0']
        # chg_df['chg' + index_name] = (chg_df['chg0'] + chg_df['chg2'] + chg_df['chg3'] + chg_df['chg4'] + chg_df['chg1'])/5
        chg_df['close_chg' + index_name] = chg_df['close_chg0']
        chg_df['excess_chg' + index_name] = chg_df['chg' + index_name] - chg_df['close_chg' + index_name]
        chg_df['net'] = (1 + chg_df['chg' + index_name]).cumprod()
        chg_df['idx'] = (1 + chg_df['close_chg' + index_name]).cumprod()
        chg_df['excess_net'] = (1 + chg_df['excess_chg' + index_name]).cumprod()
        chg_df = chg_df.reset_index(drop=False)
        pos_df = pos_df.merge(
            chg_df[['trade_date', 'chg' + index_name, 'close_chg' + index_name, 'excess_chg' + index_name]],
    ...
    pos_df = pos_df.sort_values(['trade_date'])
    pos_df['chg'] = (pos_df['chg' + name_lst[0]] + pos_df['chg' + name_lst[1]] + pos_df['chg' + name_lst[2]] + pos_df['chg' + name_lst[3]])/4
    pos_df['close_chg'] = (pos_df['close_chg' + name_lst[0]] + pos_df['close_chg' + name_lst[1]] + pos_df['close_chg' + name_lst[2]] + pos_df['close_chg' + name_lst[3]]) / 4
    pos_df['excess_chg'] = (pos_df['excess_chg' + name_lst[0]] + pos_df['excess_chg' + name_lst[1]] + pos_df[
        'excess_chg' + name_lst[2]] + pos_df[
        'excess_chg' + name_lst[3]]) / 4
    pos_df['net'] = (1 + pos_df['chg']).cumprod()
    pos_df['net'] = pos_df['net']/pos_df['net'].tolist()[0]
    pos_df['idx'] = (1 + pos_df['close_chg']).cumprod()
    pos_df['idx'] = pos_df['idx'] / pos_df['idx'].tolist()[0]
    pos_df['excess_net'] = (1 + pos_df['excess_chg']).cumprod()
    pos_df['excess_net'] = pos_df['excess_net'] / pos_df['excess_net'].tolist()[0]
    pos_df['trade_date'] = pd.to_datetime(pos_df['trade_date'])

    pos_df = pos_df.set_index(['trade_date'])
    pos_df.ix[:, ['net', 'idx', 'excess_net']].plot()
    pos_df.to_csv(fold + 'pos_df.csv')
    net_lst = pos_df.net.to_list()
    sharpe_ratio = yearsharpRatio(net_lst, 1)
    ann_return = annROR(net_lst, 1)
    max_drawdown = maxRetrace(net_lst, 1)
    title_str = 'sharpe %.2f ann_return %.2f max_drawdown %.2f' % (
